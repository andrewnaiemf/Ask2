...
class Documents extends Resource {
	 ...
	 * @param Model Class
	 */
	public static $model = \App\Models\DocumentProvider::class ;
	...
	public static function customName() {
        return __('dash.documents.documents');
	}
	...
	public function fields() {
		return [
            ...
            belongsTo()->make('provider', 'provider', Providers::class),
            text()->make('name', 'name'),
            image()
            ->make(__('dash.images'),'path')
                ->path(function($model){
                    $providerId = $model->provider->id;
                    return ('Provider/'.$providerId.'/placeImages/');
                })
                ->accept('image/*')
                ->rule('required','image'),
		];
